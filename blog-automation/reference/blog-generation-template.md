...
5. **Body** — **2,000 words minimum** (target 2,000–2,400) across **8–11 `H2` sections**. Use `H3` subsections, bullet/numbered lists, and `<strong>` emphasis. Include **at least one in-body image**. Include **at least two internal links** to the client's own pages (services, contact, resources). Reach length through genuine depth (examples, sub-topics, comparisons) — never filler. **Reading level:** write at a 6th-grade reading level — short, clear sentences and simple everyday words, technical terms explained in plain language, no collegiate or academic phrasing; voice is friendly and plain-spoken, not formal. **Anti-AI-slop (strict):** no em-dashes or en-dashes (use commas, periods, or a hyphen for number ranges), no profound-reveal sentences, no correlative-conjunction constructions ("not X, not Y, just Z"), no rule-of-three staccato fragments, no ta-da phrases; say things plainly with no fluff. *(The generator also strips stray em/en-dashes from title, excerpt, and body in code as a safety net.)*
